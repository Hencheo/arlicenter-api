...
class UserManager:
    ...
    def deactivate_user(self, cpf):
        """
        Desativa um usuário (altera status para inativo)
    ...
    def create_collection(self):
        """
        Cria a coleção de usuários no Firestore se ela não existir
        """
        try:
            ...
            logger.info(f"Coleção '{USERS_COLLECTION}' inicializada com sucesso.")
            return True
        except Exception as e:
            logger.error(f"Erro ao inicializar coleção '{USERS_COLLECTION}': {str(e)}")
            return False 